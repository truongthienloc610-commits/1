...
export function PublicNavbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/60 bg-background/80 text-foreground backdrop-blur-md">
      ...
      {/* Mobile menu */}
      {open && (
        <div className="md:hidden border-t border-border bg-background p-6 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-200">
          <nav className="flex flex-col gap-4">
            {navItems.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="text-lg font-bold text-foreground py-2 hover:text-primary"
